...
export const identificarTipoConversa = (mensagem: string): TipoConversa => {
  try {
    ...
    // ✅ DISPONIBILIDADE (deve ser AGENDAR, não CONSULTAR)
    // Ex: "qual horário tem disponível amanhã", "tem vaga amanhã de tarde"
    // Regra: se a pessoa está pedindo horários disponíveis (vagas), tratamos como fluxo de agendar.
    ...
    // CONSULTAR
    // Inclui variações tipo: "meus horários amanhã", "meus agendamentos de amanhã", etc.
    if (
    ...
    // ✅ Heurística extra: se o usuário pede "meus horários/agendamentos" mesmo sem bater nas regex acima
